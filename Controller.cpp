							...
							int channel6, int channel7, int channel8, int channel9)
{
	...
	#ifdef DEBUG
	cout << output[0] << " " << output[2] << " " << output[4] << " " << output[6];
	#endif

	libusb_interrupt_transfer(usb_device_handle, 0x01, output, 32, &transferred, 200);
	...
	libusb_interrupt_transfer(usb_device_handle, 0x81, input, 10, &transferred, 200);
